...
public class CouchbaseEventHandler implements EventHandler<CouchbaseEvent> {
	...
	private static Logger LOGGER = LoggerFactory.getLogger(CouchbaseEventHandler.class);
	...
	private void buildAndSendMutationCommandToNeo4j(MutationMessage mutationMessage) {
		...
		LOGGER.debug(jsonDocument);
		DocumentTransformer<String> remoteNeo4jTransformer = new RemoteNeo4jTransformer();
	  remoteNeo4jTransformer.transform(mutationMessage.key(), "COUCHBASE_DOCUMENT", jsonDocument);
	}

	/**
	 * TODO tell Neo4j a document has been removed
	 * 
	 * @param removeMessage
	 */
